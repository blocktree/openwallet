...
/*func (decoder *EthTxExtPara) GetGasLimit() (uint64, error) {
	gasLimit, err := strconv.ParseUint(removeOxFromHex(decoder.GasLimit), 16, 64)
	if err != nil {
...
func (decoder *AddressTxStatistic) UpdateTime() {
	now := time.Now()
	decoder.LastModifiedTime = &now
}
...
func (decoder *TransactionDecoder) GetTransactionCount2(address string) (*AddressTxStatistic, uint64, error) {
	now := time.Now()
	valid := 1
	...
	v, loaded := decoder.AddrTxStatisMap.LoadOrStore(address, t)
	...
	nonce, err := decoder.wm.GetNonceForAddress2(appendOxToAddress(address))
	if err != nil {
...
func (decoder *TransactionDecoder) GetTransactionCount(address string) (uint64, error) {
	if decoder.AddrTxStatisMap == nil {
		return 0, errors.New("map should be initialized before using.")
	}

	v, exist := decoder.AddrTxStatisMap.Load(address)
	if !exist {
...
func (decoder *TransactionDecoder) SetTransactionCount(address string, transactionCount uint64) error {
	if decoder.AddrTxStatisMap == nil {
	...
	decoder.AddrTxStatisMap.Store(address, txStatis)
	return nil
}

func (decoder *TransactionDecoder) RemoveOutdatedAddrStatic() {
	addrStatisList := make([]AddressTxStatistic, 0)
	decoder.AddrTxStatisMap.Range(func(k, v interface{}) bool {
	...
	clear := func(statis *AddressTxStatistic) {
		...
		if statis.LastModifiedTime.Before(time.Now().Add(-1 * (ADRESS_STATIS_OVERDATED_TIME * time.Minute))) {
			*statis.Valid = 0
			decoder.AddrTxStatisMap.Delete(statis)
		}
	}
...
func (decoder *TransactionDecoder) RunClearAddrStatic() {
	go func() {
		for {
			time.Sleep(5 * time.Minute)
			decoder.RemoveOutdatedAddrStatic()
		}
	}()
}
...
func (decoder *TransactionDecoder) CreateSimpleRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	amount, err := ConvertNasStringToWei(amountStr) //ConvertToBigInt(amountStr, 16)
	if err != nil {
...
func (decoder *TransactionDecoder) CreateNRC20TokenRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	//check交易交易单基本字段
	err := VerifyRawTransaction(rawTx)
	if err != nil {
		openwLogger.Log.Errorf("Verify raw tx failed, err=%v", err)
	...
	err = decoder.wm.GetTokenBalanceByAddress(rawTx.Coin.Contract.Address, addrsBalanceIfList...)
	if err != nil {
	...
	for i, _ := range addrsBalanceList {
		...
		if addrsBalanceList[i].TokenBalance.Cmp(amount) > 0 {
			...
			fee, err = decoder.wm.GetTransactionFeeEstimated(addrsBalanceList[i].Address, rawTx.Coin.Contract.Address, nil, data)
			if err != nil {
				openwLogger.Log.Errorf("GetTransactionFeeEstimated from[%v] -> to[%v] failed, err=%v", addrsBalanceList[i].Address, to, err)
				return err
			}
	...
	_, nonce, err := decoder.GetTransactionCount2(keySignList[0].Address.Address)
	if err != nil {
		openwLogger.Log.Errorf("GetTransactionCount2 failed, err=%v", err)
		return err
	}

	signer := types.NewEIP155Signer(big.NewInt(int64(decoder.wm.GetConfig().ChainID)))
...
func (decoder *TransactionDecoder) CreateRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	return decoder.CreateNRC20TokenRawTransaction(wrapper, rawTx)
}
...
func (decoder *TransactionDecoder) SubmitSimpleRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	txStatis, _, err := decoder.GetTransactionCount2(from)
	if err != nil {
	...
	err = func() error {
		...
		txid, err := decoder.wm.WalletClient.ethSendRawTransaction(common.ToHex(rawTxPara))
		if err != nil {
...
func (decoder *TransactionDecoder) SubmitErc20TokenRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	//check交易交易单基本字段
	err := VerifyRawTransaction(rawTx)
	if err != nil {
		openwLogger.Log.Errorf("Verify raw tx failed, err=%v", err)
...
func (decoder *TransactionDecoder) SubmitRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	return decoder.SubmitErc20TokenRawTransaction(wrapper, rawTx)
}
